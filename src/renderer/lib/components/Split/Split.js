...
(function (global, factory) {
  ...
  var Split = function (ids, options) {
    ...
    var elementStyle =
      ...
      function (dim, size, gutSize) {
        ...
        if (typeof size !== 'string' && !(size instanceof String)) {
          if (!isIE8) {
            // console.log('split 118',calc, size)
            style[dim] = calc + '(' + Number(size) + '% - ' + gutSize + 'px)';
          } else {
            style[dim] = size + '%';
          }
        } else {
          style[dim] = Number(size);
        }
    ...
    function adjust(offset) {
      ...
      var percentage = Number(Number(a.size) + Number(b.size));
      a.size = Number((offset / this.size) * percentage);
      b.size = Number(Number(percentage) - Number(offset / this.size) * Number(percentage));
    ...
    function drag(e) {
      ...
      if (offset <= Number(elements[this.a].minSize + snapOffset + this.aGutterSize)) {
        offset = Number(elements[this.a].minSize + this.aGutterSize);
      } else if (offset >= Number(this.size) - Number(elements[this.b].minSize) + Number(snapOffset) + Number(this.bGutterSize)) {
        offset = Number(this.size - Number(Number(elements[this.b].minSize) + Number(this.bGutterSize)));
      }
    ...
    function calculateSizes() {
      ...
      this.size = Number(Number(a[getBoundingClientRect]()[dimension]) + Number(b[getBoundingClientRect]()[dimension]) + Number(this.aGutterSize) + Number(this.bGutterSize));
    ...
    elements = ids.map(function (id, i) {
      ...
      if (i === 0 || i === ids.length - 1) {
        setElementSize(element.element, Number(element.size), gutterSize / 2);
      } else {
        setElementSize(element.element, Number(element.size), gutterSize);
      }
    ...
    function setSizes(newSizes) {
      newSizes.forEach(function (newSize, i) {
        if (i > 0) {
          ...
          a.size = Number(newSizes[i - 1]);
          b.size = Number(newSize);
    ...
    return {
      ...
      gutterVis: function (op, num) {
        ...
        for (let i = s; i < e; i++) {
          var pair = pairs[i];
          if (op) {
            pair.gutter.style.display = 'block';
          } else {
            pair.gutter.style.display = 'none';
          }
        }
        //console.log(pair,op)
      },
