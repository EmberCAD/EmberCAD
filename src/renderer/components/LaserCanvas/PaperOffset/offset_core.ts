...
function makeRoundJoin(segment1: paper.Segment, segment2: paper.Segment, originPoint: paper.Point, radius: number) {
  const through = segment1.point.subtract(originPoint).add(segment2.point.subtract(originPoint)).normalize(Math.abs(radius)).add(originPoint);
  ...
  if (origin.isSmooth()) {
    ...
    if (intersection.length === 0) {
      if (distance > Math.abs(offset) * 0.1) {
        ...
        switch (joinType) {
          case 'miter':
            const join = getPointLineIntersections(curve1.point2, curve1.point2.add(curve1.getTangentAtTime(1)), curve2.point1, curve2.point1.add(curve2.getTangentAtTime(0)));
            // prevent sharp angle
...
function removeIntersection(path: PathType) {
  if (path.closed) {
    ...
    if (newPath instanceof paper.CompoundPath) {
      (newPath.children as paper.Path[])
        .filter((c) => {
...
function offsetSimpleShape(path: paper.Path, offset: number, join: StrokeJoinType, limit: number): PathType {
  ...
  let segments = connectBeziers(raws, join, source, offset, limit);
  if (!segments) return;
  ...
  switch (cap) {
    case 'round':
      ...
      const result = new paper.Path({
        segments: [...heads, ...oSegments, ...tails, ...iSegments],
        closed: true,
        insert: false
      });
      result.reduce({});
...
export function offsetPath(path: PathType, offset: number, join: StrokeJoinType, limit: number): PathType | null {
