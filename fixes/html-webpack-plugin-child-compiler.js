...
function extractHelperFilesFromCompilation (mainCompilation, childCompilation, filename, childEntryChunks) {
  ...
  const helperContents = helperAssetNames.map((helperFileName, index) => {
    const safeSource = (asset) => {
      if (!asset) {
        return undefined;
      }
    ...
    const directAsset = childCompilation.assets && childCompilation.assets[helperFileName];
    const directSource = safeSource(directAsset);
    if (directSource !== undefined) {
      return directSource;
    }

    if (typeof childCompilation.getAsset === 'function') {
      const assetInfo = childCompilation.getAsset(helperFileName);
      const assetFromInfo = assetInfo && (assetInfo.source || assetInfo.content);
      const infoSource = safeSource(assetFromInfo);
      if (infoSource !== undefined) {
        return infoSource;
      }
    }

    const entryChunk = childEntryChunks && childEntryChunks[index];
    if (entryChunk && Array.isArray(entryChunk.files)) {
      const fallbackFileName = entryChunk.files.find((fileName) => {
        return childCompilation.assets && childCompilation.assets[fileName];
      });
      if (fallbackFileName) {
        const fallbackAsset = childCompilation.assets[fallbackFileName];
        const fallbackSource = safeSource(fallbackAsset);
    ...
    const errorMessage = `HtmlWebpackPlugin: helper asset "${helperFileName}" missing in child compilation`;
    if (mainCompilation && Array.isArray(mainCompilation.errors)) {
      mainCompilation.errors.push(new Error(errorMessage));
    }
