...
var paper = function (self, undefined) {
  ...
  var Numerical = new (function () {
    var abscissas = [
      ...
      [
        0.1834346424956498049394761, 0.525532409916328985817739, 0.7966664774136267395915539,
      ...
      [
        0.148874338981631210884826, 0.4333953941292471907992659, 0.6794095682990244062343274,
        0.8650633666889845107320967, 0.973906528517171720077964,
      ],
      [
    ...
    var weights = [
      ...
      [
        0.417959183673469387755102, 0.3818300505051189449503698, 0.2797053914892766679014678,
        0.1294849661688696932706114,
      ],
      [
        0.3626837833783619829651504, 0.3137066458778872873379622, 0.222381034453374470544356,
      ...
      [
        0.295524224714752870173893, 0.2692667193099963550912269, 0.2190863625159820439955349,
    ...
    statics: new (function () {
      ...
      function createPath(segments, closed, args) {
        var props = Base.getNamed(args),
          path = new Path(
            props && (props.insert == true ? Item.INSERT : props.insert == false ? Item.NO_INSERT : null),
          );
        path._add(segments);
        path._closed = closed;
  ...
  var DomElement = new (function () {
    ...
    return {
      ...
      isInView: function (el) {
        return (
          !DomElement.isInvisible(el) && DomElement.getViewportBounds(el).intersects(DomElement.getBounds(el, true))
        );
      },
  ...
  new (function () {
    ...
    function exportText(item) {
      ...
      let txt = item._content;
      let m = item._matrix;
      if (item._content.indexOf('\n') >= 0) {
        let lines = txt.split('\n');
        lines.forEach((line, i) => {
          let atr = {};
          atr['x'] = `${m.tx / m.scaling.x}`;
          atr['dy'] = `${i * 1.2}em`;
          let tspan = SvgElement.create('tspan', atr, formatter);
      ...
      (function (root, mod) {
        ...
        function parseIdent(liberal) {
          ...
          if (tokType === _name) {
            if (
              !liberal &&
              ((options.forbidReserved && (options.ecmaVersion === 3 ? isReservedWord3 : isReservedWord5)(tokVal)) ||
